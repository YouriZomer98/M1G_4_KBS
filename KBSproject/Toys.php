...
include 'sidebarToys.php';?>
<div class="main">
<div class="container">
    <div id="products" class="row">
        <?php
        if(isset($_GET['submit'])){
            $sql = "SELECT * FROM stockitems WHERE StockItemID IN (SELECT StockItemID FROM stockitemstockgroups WHERE StockGroupID = 9)";
            ...
            if(isset($_GET['merk']) && !empty($_GET['merk'])){
                $merken = implode("', '", $_GET['merk']);
                $sql = "SELECT * FROM stockitems WHERE Brand IN ('$merken')";
                $merkarray = dbSelectAll($sql);
                $merk = $merkarray->fetchAll(PDO::FETCH_COLUMN);
            }else{
                $sql = "SELECT * FROM stockitems";
                $merkarray = dbSelectAll($sql);
                $merk = $merkarray->fetchAll(PDO::FETCH_COLUMN);
            ...
            $samen = array_intersect($alles, $kleur, $maat, $merk);
            $productID = implode("', '", $samen);
            $sql = "SELECT * FROM stockitems WHERE StockItemID IN ('$productID') ORDER BY $orderby";
            $product = dbSelectAll($sql);
        }else{$sql = "SELECT * FROM stockitems WHERE StockItemID IN (SELECT StockItemID FROM stockitemstockgroups WHERE StockGroupID = 9)";
        $product = dbSelectAll($sql);}
        $uniq = array();
        $k = 0;
