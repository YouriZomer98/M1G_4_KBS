...
include 'sidebarMugs.php';?>
<div class="main">
<div class="container">
    <div id="products" class="row">
        <?php
        if(isset($_GET['submit'])){
            $sql = "SELECT * FROM stockitems WHERE StockItemID IN (SELECT StockItemID FROM stockitemstockgroups WHERE StockGroupID = 3)";
        ...
        }else{$sql = "SELECT * FROM stockitems WHERE StockItemID IN (SELECT StockItemID FROM stockitemstockgroups WHERE StockGroupID = 3)";
        $product = dbSelectAll($sql);}
        $uniq = array();
        $k = 0;
